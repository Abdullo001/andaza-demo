...
export default {
  ...
  data() {
    return {
      ...
      dialogStatus: 'add',
      headers: [
        {text: this.$t('readyWarehouse.documents.type'), align: 'start', sortable: false, value: 'extension'},
        ...
        {text: this.$t('readyWarehouse.documents.date'), sortable: false, value: 'createdAt'},
        {text: this.$t('readyWarehouse.documents.action'), sortable: false, align: 'center', value: 'actions'},
      ],
  ...
  methods: {
    ...mapActions({
      getDocuments: "documents/getStockDocuments",
    ...
    editDocument(item) {
      ...
      this.edit_document.stockId = this.uid;
    ...
    async deleteDocumentRow() {
      ...
      await this.deleteDocument({docId: this.currentId, stockId: this.uid});
    ...
    async addDocument() {
      ...
      if(valid) {
          ...
          this.newDocument.stockId = this.uid
          delete this.newDialog.modelId
  ...
  async mounted() {
    ...
    if (id !== 'add-garment') {
      await this.getDocuments({stockId: id, fileType: 'DOC'});
