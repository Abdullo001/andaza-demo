<template>
  <div>
    ...
    <v-card color="#fff" elevation="0" class="rounded-t-lg pb-4 mb-4">
      <v-card-title>
        <div>
      ...
      <v-form lazy-validation v-model="valid_search" ref="filter_form">
        <v-row class="mx-0 px-0 mb-2 mt-4 pa-4 w-full" justify="start">
          <v-col cols="12" lg="3" md="3">
            <div class="label">
              {{ $t("planningProduction.dialog.orderNumber") }}
            </div>
            <v-combobox
    ...
    <v-dialog v-model="fillingDialog" width="500">
      <v-card>
        <v-card-title class="d-flex justify-space-between w-full">
          <div class="text-capitalize font-weight-bold">Filling</div>
          <v-btn icon color="#544B99" @click="fillingDialog = false">
        ...
        <v-card-text class="mt-4">
          <v-form ref="spend_form" v-model="spend_validate" lazy-validation>
            <v-row class="mx-0 px-0 mb-2 mt-4 pa-4 w-full" justify="start">
              ...
              <v-col cols="12" lg="6" md="3">
                <div class="label">
                  {{ $t("prefinances.child.modelNumber") }}
                </div>
                <v-combobox
              ...
              <v-col cols="12" lg="6" md="3">
                <div class="label">
                  {{ $t("catalogAccessory.table.specification") }}
                </div>
                <v-combobox
                  v-model="spend.accessoryIdTo"
                  :items="accessoriesSpendList"
        ...
        <v-card-text class="mt-4">
          ...
          >
            <v-row>
              ...
              <v-col cols="12">
                <div class="label">
                  {{ $t("planningProduction.planning.quantity") }}
                </div>
                <v-text-field
                  :rules="[formRules.required]"
      ...
      <v-card>
        ...
        <v-card-text class="mt-4">
          ...
          >
            ...
            <v-row v-if="withSizes">
              <v-col
                v-for="(item, idx) in subcontractor.editableSizeDistributions"
                :key="idx"
                cols="3"
              >
                <div class="label">{{ item.size }}</div>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        {
          text: this.$t("warehouseId.name"),
          value: "accessoryName",
          sortable: false,
        },
        {
          text: this.$t("warehouseId.specification"),
          value: "specification",
          sortable: false,
        },
        ...
        {
          text: this.$t("warehouseId.spentQuantity"),
          value: "spentQuantity",
          sortable: false,
        },
        {
          text: this.$t("warehouseId.perUnitPrice"),
          value: "price",
          sortable: false,
        },
        {
          text: this.$t("warehouseId.totalPrice"),
          value: "totalPrice",
          sortable: false,
        },
        {
        ...
        {
          text: this.$t("warehouseId.orderedDate"),
          value: "orderedDate",
          sortable: false,
        },
        {
          text: this.$t("warehouseId.actions"),
          value: "actions",
          sortable: false,
        },
      ],
  ...
  watch: {
    ...
    "spend.modelId"(val) {
      if (!!val && val !== "") {
        if (!!this.spend.orderId && this.spend.orderId !== "") {
          this.searchSpendAccessory({
            orderId: this.spend.orderId.id,
            modelId: val.id,
          });
        }
      }
    ...
    accessoriesDetailList(list) {
      const specialList = list.map(function (el) {
        const value = {};
        const sizesList = [];
        el?.remainingSizeDistributions?.forEach((item) => {
  ...
  methods: {
    ...
    setDeliviredQuantity(item) {
      ...
      this.setDelivered({
        ...
        orderId: this.filters.orderId?.id,
      });
    },
    search() {
    ...
    deleteItem() {
      if (id !== "add-accessory-warehouse") {
        this.deleteAccessory({
          planningOrderId: this.selectedItem,
          modelId: this.filters.modelId.id,
          orderId: this.filters.orderId.id,
        });
      } else {
    ...
    async saveWorkshop() {
      ...
      if (this.withSizes) {
        data.sizeDistributions =
          this.selectedItem.editableSizeDistributions.map((item) => ({
            size: item.size,
    ...
    async saveSubcontract() {
      ...
      if (this.withSizes) {
        data.sizeDistributions =
          this.subcontractor.editableSizeDistributions.map((item) => ({
            size: item.size,
            quantity: item.quantity ? item.quantity : 0,
          }));
      }
      data.partnerId = this.subcontractor.partnerId?.id;
      await this.giveSubcontractor({
        data,
        modelId: this.filters.modelId?.id,
        orderId: this.filters.orderId?.id,
      });
