<template>
  <div>
      ...
      <v-form lazy-validation v-model="valid_search" ref="filter_form">
        <v-row class="mx-0 px-0 mb-7 mt-4 pa-4 w-full" justify="start">
          <v-col cols="12" lg="2" md="2">
            <v-text-field label="Sip №" outlined class="rounded-lg filter" v-model.trim="filters.sipNumber" hide-details
              dense @keydown.enter="filterData" />
          </v-col>
          <v-col cols="12" lg="2" md="2">
            <v-text-field label="Batch №" outlined class="rounded-lg filter" v-model.trim="filters.batchNumber"
              hide-details dense @keydown.enter="filterData" />
          </v-col>
          <v-col cols="12" lg="2" md="2">
            <v-text-field label="Order №" outlined class="rounded-lg filter" v-model.trim="filters.orderNumber"
              hide-details dense @keydown.enter="filterData" />
          </v-col>
          <v-spacer />
          <v-col cols="12" lg="4">
    ...
    <v-data-table class="mt-4 rounded-lg pt-4" :headers="headers" :items="current_list" :items-per-page="10"
      ...
      }" show-expand :single-expand="singleExpand" :expanded.sync="expanded" @item-expanded="loadDetails">
      <template #top>
        <v-toolbar elevation="0">
          <v-toolbar-title class="d-flex w-full align-center justify-space-between">
            <div>Fabric warehouse</div>
            <div>
              <v-btn color="#7631FF" outlined class="text-capitalize rounded-lg mr-2"
                @click="$router.push(`/fabric-warehouse/fabric-stock`)">
                Fabric stock
              </v-btn>
              <v-btn color="#7631FF" dark class="text-capitalize rounded-lg" @click="addArrivedFabric">
        ...
        <div class="d-flex ">
          <v-tooltip top color="#7631FF" class="pointer" v-if="Object.keys(item).length > 2">
            <template #activator="{ on, attrs }">
              <v-btn icon v-bind="attrs" v-on="on" color="#7631FF" @click="spendFunc(item)">
          ...
          <v-tooltip top color="#7631FF" class="pointer" v-if="Object.keys(item).length > 2">
            <template #activator="{ on, attrs }">
              <v-btn icon v-bind="attrs" v-on="on" color="#7631FF" @click="getHistory(item)">
      ...
      <v-card>
          ...
          <v-form ref="new_form" v-model="new_validate" lazy-validation>
            <v-row>
              <v-col cols="12" lg="6">
                ...
                <v-select append-icon="mdi-chevron-down" v-model="arrivedFabric.fabricOrderId"
                  :rules="[formRules.required]" :items="sipNumbers" item-text="sipNumber" item-value="fabricOrderId"
                  hide-details class=" base rounded-lg" rounded outlined dense placeholder="Select Sip №" />
              </v-col>
              <v-col cols="12" lg="6">
                <div class="label">Batch №</div>
                <v-text-field :rules="[formRules.required]" v-model="arrivedFabric.batchNumber" outlined hide-details
                  dense class="rounded-lg base " placeholder="Enter batch №" color="#7631FF" />
              </v-col>
              <v-col cols="12" lg="6">
                <div class="label">Fabric width in fact (cm)</div>
                <v-text-field :rules="[formRules.required]" v-model="arrivedFabric.fabricWidthInFact" outlined
                  hide-details dense class="rounded-lg base " placeholder="Enter fabric widht in fact" color="#7631FF" />
              </v-col>
              <v-col cols="12" lg="6">
                <div class="label">Density (gsm) in fact gr/m2</div>
                <v-text-field :rules="[formRules.required]" v-model="arrivedFabric.densityInFact" outlined hide-details
                  class="rounded-lg base " height="44" dense color="#7631FF" placeholder="Enter density (gsm) in fact" />
              </v-col>
              <v-col cols="12" lg="6">
                <div class="label">Fact received Gross weight</div>
                <v-text-field :rules="[formRules.required]" v-model="arrivedFabric.factReceivedGrossWeight" outlined
                  hide-details dense class="rounded-lg base " placeholder="Enter fact received gross weight"
                  color="#7631FF" />
              </v-col>
              <v-col cols="12" lg="6">
                <div class="label">Fact received Netto weight</div>
                <v-text-field :rules="[formRules.required]" v-model="arrivedFabric.factReceivedNettoWeight" outlined
                  hide-details dense class="rounded-lg base " placeholder="Enter fact received netto weight"
                  color="#7631FF" />
              </v-col>
            </v-row>
          </v-form>
        ...
        <v-card-actions class="d-flex justify-center pb-8">
          ...
          <v-btn v-if="title === 'New'" class="rounded-lg text-capitalize ml-4 font-weight-bold" color="#7631FF" dark
            width="163" @click="saveArrivedFabric">
            save
          </v-btn>
          ...
          <v-form ref="spend_form" v-model="spend_validate" lazy-validation>
            <v-row>
              <v-col cols="12">
                ...
                <v-select append-icon="mdi-chevron-down" v-model="spendingFabric.idTo" :rules="[formRules.required]"
                  :items="toSipNumbers" item-text="sipNumber" item-value="id" hide-details color="#7631FF"
                  class=" base rounded-lg" rounded outlined dense placeholder="Select Sip №" />
              </v-col>
              <v-col cols="12">
                <div class="label">to Batch №</div>
                <v-select append-icon="mdi-chevron-down" v-model="spendingFabric.idTo" :rules="[formRules.required]"
                  :items="toSipNumbers" item-text="batchNumber" item-value="id" hide-details class=" base rounded-lg"
                  rounded color="#7631FF" outlined dense placeholder="Batch №" />
              </v-col>
              <v-col cols="12">
                <div class="label">Spending quantity</div>
                <v-text-field :rules="[formRules.required]" v-model="spendingFabric.spendingQuantity" outlined
                  hide-details dense class="rounded-lg base " placeholder="Enter spending quantity" color="#7631FF"
                  suffix="kg" />
              </v-col>
            </v-row>
          </v-form>
          ...
          <v-form ref="workshop_form" v-model="workshop_validate" lazy-validation>
            <v-row>
              <v-col cols="12">
                ...
                <v-select append-icon="mdi-chevron-down" v-model="workshop.modelNumber" :rules="[formRules.required]"
                  :items="modelNumbers" hide-details color="#7631FF" class=" base rounded-lg" rounded outlined dense
                  placeholder="Select Model №" />
              </v-col>

              <v-col cols="12">
                <div class="label">Giving fabric quantity</div>
                <v-text-field :rules="[formRules.required]" v-model="workshop.quantity" outlined hide-details dense
                  class="rounded-lg base " placeholder="Enter giving fabric quantity" color="#7631FF"
          ...
          <v-form ref="subcontractor_form" v-model="subcontractor_validate" lazy-validation>
            <v-row>
              <v-col cols="12">
                ...
                <v-select append-icon="mdi-chevron-down" v-model="subcontractor.partnerId" :items="partnerList"
                  :rules="[formRules.required]" item-text="name" item-value="id" hide-details color="#7631FF"
                  class=" base rounded-lg" rounded outlined dense placeholder="Select partner" />
              </v-col>

              <v-col cols="12">
                <div class="label">Model №</div>
                <v-select append-icon="mdi-chevron-down" v-model="subcontractor.modelNumber" :items="modelNumbers"
                  :rules="[formRules.required]" hide-details color="#7631FF" class=" base rounded-lg" rounded outlined
                  dense placeholder="Select Model №" />
              </v-col>

              <v-col cols="12">
                <div class="label">Giving fabric quantity</div>
                <v-text-field :rules="[formRules.required]" v-model="subcontractor.quantity" outlined hide-details dense
                  class="rounded-lg base " placeholder="Enter giving fabric quantity" color="#7631FF"
    ...
    <v-dialog v-model="history_dialog" max-width="800">
      <v-card flat>
        ...
        <v-card-text>
          <v-data-table :headers="historyHeaders" :items="historyList" hide-default-footer class="mt-4 rounded-lg"
